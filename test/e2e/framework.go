...
func (data *TestData) deploySparkOperator() error {
	...
	if err != nil || rc != 0 {
		return fmt.Errorf("error when deploying the Spark Operator YML %s: %s, %s, %v", flowVisibilityYML, stdout, stderr, err)
	}
...
func teardownFlowVisibility(tb testing.TB, data *TestData) {

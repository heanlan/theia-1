...
// getSparkOperator retrieves the name of the Spark Operator Pod (policy-reco-spark-operator-*).
...
// deploySparkOperator deploys the Spark Operator in the flow-visibility namespace
func (data *TestData) deploySparkOperator() error {
	rc, stdout, stderr, err := data.provider.RunCommandOnNode(controlPlaneNodeName(), fmt.Sprintf("kubectl apply -f %s -n %s", sparkOperatorYML, flowVisibilityNamespace))
	if err != nil || rc != 0 {
		return fmt.Errorf("error when deploying the Spark Operator YML %s: %s, %s, %v", sparkOperatorYML, stdout, stderr, err)
	}
...
func (data *TestData) deleteFlowVisibility() error {
	...
	rc, _, stderr, err := data.provider.RunCommandOnNode(controlPlaneNodeName(), fmt.Sprintf("kubectl delete -f %s", flowVisibilityYML))
	if err != nil || rc != 0 {
...
func teardownPolicyRecommendation(tb testing.TB, data *TestData) {
	...
	rc, _, stderr, err := data.provider.RunCommandOnNode(controlPlaneNodeName(), fmt.Sprintf("kubectl delete -f %s -n %s", sparkOperatorYML, flowVisibilityNamespace))
	if err != nil || rc != 0 {
		tb.Logf("Error when deleting Spark Operator: %v, stderr: %s", err, stderr)
	}
